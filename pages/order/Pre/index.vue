<template>
	<view class="content">
		...
		<view class="nav">
			...
			<view @click="change(2)" class="nav-ul"><text :class="[btnnum==2?'active':'text']">预约成功</text></view>
			<view @click="change(3)" class="nav-ul"><text :class="[btnnum==3?'active':'text']">已完成</text></view>
		...
		<view class="order-box">
			<view class="order-list" v-for="(item,index) in order" :key="index">
				<view class="order-list-top">
					...
					<view class="order-state" style="color: #FF0000;" v-if="item.state==2">预约成功</view>
					<view class="order-state" style="color: #FF0000;" v-if="item.state==3">交易成功</view>
				...
				<view class="order-list-bottom" v-if="item.state==2">
					<view class="gaiqian" style="border: 1rpx solid #E2E1E1;color: #131313;">再次预约</view>
					<view class="gaiqian" style="border: 1rpx solid #E2E1E1;color: #131313;">取消预约</view>
					<view class="shiyong">去使用</view>
					<!-- <view class="shiyong">去使用</view> -->
				</view>
		...
		data() {
			return {
				...
				order:[
						...
						goods_name:"佛顶山生态旅游景区门票",
						state:1,
						goods_img:"../../../static/banner.png",
						goods_number:2,
						goods_time:"2020-5-19 14:43:59",
						money:100,
						pay:5.2
					},
					{
